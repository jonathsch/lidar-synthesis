...
from lidar_synthesis.models.lidar2steering import LitLidar2Steering

MODEL_PATH = Path("model_ckpt/lidar_steering/best.ckpt").resolve()
...
class LidarAgent(autonomous_agent.AutonomousAgent):
    ...
    def setup(self, path_to_conf_file):
        ...
        # Load model
        self.model = LitLidar2Steering.load_from_checkpoint(MODEL_PATH).to("cuda")
        self.model.eval()
    ...
    def run_step(self, input_data, timestamp):
        ...
        # Run inference
        steer = self.model(lidar).detach().cpu().numpy().item()
